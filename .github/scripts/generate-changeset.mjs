#!/usr/bin/env node
// Generates a changeset from the Conventional Commits found in a git range,
// as a fallback for whoever doesn't write one by hand.
//
...
//
// Each package's bump comes only from the commits that touch it: a chore
// commit riding along in the same range never drags a package into the
// release, and a package touched by both a feat and a fix in the same range
// gets the more significant of the two, not the more significant across the
// whole range.
//
// Usage:
//   node generate-changeset.mjs --from <ref> --to <ref> [--out-dir <dir>] [--filename <name>]
//
// Writes <out-dir>/<filename> (default: auto-<short sha>.md) when at least
// one feat or fix commit touches a package, and reports the outcome on
// $GITHUB_OUTPUT as `created` (true|false) and, when true, `path`.
...
/** null when the commit does not carry a release-worthy Conventional Commit type. */
